...
class YTDLSource(nextcord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search=None, url=None, *, loop: asyncio.BaseEventLoop = None):
        ...
        return cls(ctx, nextcord.FFmpegPCMAudio(song_url, **cls.FFMPEG_OPTIONS), data=info)
...
class Song:
    ...
    def create_embed(self):
        ...
        embed = (nextcord.Embed(title=embedMessage[0],
                               description='[{0.source.title}]({0.source.url})'.format(
                                   self),
                               color=nextcord.Colour.from_rgb(r(), r(), r()))
...
class VoiceState:
    ...
    async def audio_player_task(self):
        while True:
            ...
            message:nextcord.message = await self.current.source.channel.send(embed=self.current.create_embed())
...
class Moderation(commands.Cog):
    ...
    async def mute(self,ctx, arg):
      simp = ctx.message.mentions
      for user in simp:
        await user.add_roles(ctx.guild.get_role(903467851812315137))
    @commands.command()
    @commands.has_permissions(manage_guild=True)
    async def unmute(self,ctx, arg):
      simp = ctx.message.mentions
      for user in simp:
        await user.remove_roles(ctx.guild.get_role(903467851812315137))
